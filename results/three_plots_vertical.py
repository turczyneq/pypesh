...
fig, axes = plt.subplots(3, 1, figsize=(10.5 * 0.6, 10 * 0.6), sharex=True)
...
for i, x in enumerate([r"(a)", r"(b)", r"(c)"]):
    axes[i].text(0.01, 0.91, x, transform=axes[i].transAxes, fontsize=fontsize)
...
for ax in axes:
    ...
    ax.set_aspect(0.35 * x_range / y_range)
...
axes[0].set_ylabel(r"$\textrm{\textit{Sh}}$", fontsize=fontsize)
axes[1].set_ylabel(r"$\widetilde{\textrm{\textit{Sh}}}$", fontsize=fontsize)
axes[2].set_ylabel(
...
tosave = parent_dir / "graphics/big_plot_vertical.pdf"
