...
def proces(arg):
    ...
    return peclet, ball_radius, femsol, integral, xargs, sol
...
for result in pool.map(proces, args):
    peclet, ball_radius, femsol, integral, xargs, sol = result
    with open(output_file, "a") as f:
        f.write(f"{peclet}\t{ball_radius}\t{femsol}\t{integral}")
        for arg in xargs:
            f.write(f"\t{arg}")
        for arg in sol:
            f.write(f"\t{arg}")
        f.write(f"\n")
    print(f"apended sol for peclet = {peclet} and ball = {ball_radius}") 
